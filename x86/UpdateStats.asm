
macro update_continuation_histories ss, offset, W_bonus, absbonus, t1
	; W_bonus is 32*bonus --> 1*bonus
	; absbonus is abs(bonus)
	; clobbers rax, rcx, rdx, t1
  local over1, over2, over3
	     Assert   b, absbonus, 324, 'assertion abs(bonus)<324 failed in update_continuation_histories'

		mov   t1, qword[ss-1*sizeof.State+State.counterMoves]
		cmp   dword[ss-1*sizeof.State+State.currentMove], 1
		 jl   over1
	apply_bonus   (t1+4*(offset)), W_bonus, absbonus, 29952
over1:

		mov   t1, qword[ss-2*sizeof.State+State.counterMoves]
		cmp   dword[ss-2*sizeof.State+State.currentMove], 1
		 jl   over2
	apply_bonus   (t1+4*(offset)), W_bonus, absbonus, 29952
over2:

		mov   t1, qword[ss-4*sizeof.State+State.counterMoves]
		cmp   dword[ss-4*sizeof.State+State.currentMove], 1
		 jl   over3
	apply_bonus   (t1+4*(offset)), W_bonus, absbonus, 29952
over3:
end macro




macro update_quiet_stats move, quiets, quietsCnt, W_bonus, absbonus, prevOffset
	; clobbers rax, rcx, rdx, r8, r9
	; it also might clobber rsi and change the sign of W_bonus
  local DontUpdateKillers, DontUpdateOpp, BonusTooBig, NextQuiet, Return


  if DEBUG
		mov   eax, dword[rbx-1*sizeof.State+State.currentMove]
		and   eax, 63
	      movzx   ecx, byte[rbp+Pos.board+rax]
		shl   ecx, 6
		add   eax, ecx
	     Assert   e, prevOffset, rax, 'assertion prevOffset = offset of [piece_on(prevSq),prevSq] failed in update_quiet_stats'
  end if


		mov   eax, dword[rbx+State.killers+4*0]
		cmp   eax, move
		 je   DontUpdateKillers
		mov   dword[rbx+State.killers+4*1], eax
		mov   dword[rbx+State.killers+4*0], move
DontUpdateKillers:

		cmp   dword[rbx-1*sizeof.State+State.currentMove], 1
		 jl   DontUpdateOpp
		mov   r8, qword[rbp+Pos.counterMoves]
		mov   dword[r8+4*prevOffset], move
DontUpdateOpp:

		imul   W_bonus, absbonus, 1
		; cmp   absbonus, 324
		; jae   BonusTooBig

		mov   eax, move
		and   eax, 64*64-1
		mov   r8d, dword[rbp+Pos.sideToMove]
		shl   r8d, 12+2
		add   r8, qword[rbp+Pos.history]
		lea   r8, [r8+4*rax]
	apply_bonus   r8, W_bonus, absbonus, 10368

		mov   r9d, move
		and   r9d, 63
		mov   eax, move
		shr   eax, 6
		and   eax, 63
	      movzx   eax, byte[rbp+Pos.board+rax]
		shl   eax, 6
		add   r9d, eax
      update_continuation_histories   (rbx-0*sizeof.State), r9, W_bonus, absbonus, r8


  match =0, quiets
  else
	; Decrease all the other played quiet moves
		neg   W_bonus
		xor   esi, esi
		cmp   esi, quietsCnt
		 je   Return
NextQuiet:
		mov   edx, dword[quiets+4*rsi]
		mov   ecx, edx
		mov   eax, edx

		and   edx, 64*64-1
		mov   r8d, dword[rbp+Pos.sideToMove]
		shl   r8d, 12+2
		add   r8, qword[rbp+Pos.history]
		lea   r8, [r8+4*rdx]

		and   ecx, 63
		shr   eax, 6
		and   eax, 63
		movzx   eax, byte[rbp+Pos.board+rax]
		shl   eax, 6
		lea   r9d, [rax+rcx]

		apply_bonus   r8, W_bonus, absbonus, 10368

		update_continuation_histories   (rbx-0*sizeof.State), r9, W_bonus, absbonus, r8

		add   esi, 1
		cmp   esi, quietsCnt
		 jb   NextQuiet
  end match

BonusTooBig:
Return:
end macro


macro update_capture_stats move, captures, captureCnt, W_bonus, absbonus
	; clobbers rax, rcx, rdx, r8, r9
	; it also might clobber rsi, r11
  local BonusTooBig, NextCapture, Return

            imul  W_bonus, absbonus, 1
            mov  r9, qword[rbp + Pos.captureHistory]
            ; cmp  absbonus, 324
            ; jae  BonusTooBig

            test r8b, dl
            jz   @1f

            mov  eax, move
            mov  ecx, move
            shr  ecx, 6
            and  eax, 63
            and  ecx, 63
          movzx  ecx, byte[rbp + Pos.board + rcx]
            shl  ecx, 6
            add  ecx, eax
          movzx  eax, byte[rbp + Pos.board + rax]
            and  eax, 7
            shl  ecx, 3
            add  ecx, eax
            lea  r8, [r9 + 4*rcx]
    apply_bonus  r8, W_bonus, absbonus, 10368

@1:
  match =0, quiets
  else
            neg  W_bonus
            xor  esi, esi
            cmp  esi, captureCnt
             je  Return
NextCapture:
            mov  eax, dword[captures + 4*rsi]
            mov  ecx, dword[captures + 4*rsi]
            shr  ecx, 6
            and  eax, 63
            and  ecx, 63
            lea  esi, [rsi + 1]
          movzx  ecx, byte[rbp + Pos.board + rcx]
            shl  ecx, 6
            add  ecx, eax
          movzx  eax, byte[rbp + Pos.board + rax]
            and  eax, 7
            shl  ecx, 3
            add  ecx, eax
            lea  r8, [r9 + 4*rcx]
    apply_bonus  r8, W_bonus, absbonus, 10368
            cmp  esi, captureCnt
             jb  NextCapture
  end match

BonusTooBig:
Return:
end macro
