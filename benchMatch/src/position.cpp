...
void PrintPieceType (Piece pieceType) {
	switch (pieceType)
	{
	case W_PAWN: // Pawn
		std::cerr << "P";
...
void Position::init() {
  ...
  unsigned long long pseudoAttacksHash = 0;
  ...
  for (Piece pc : Pieces)
  {
	  ...
	  for (Square s1 = SQ_A1; s1 <= SQ_H8; ++s1)
	  {
		  ...
		  {
			  if (PseudoAttacks[type_of(pc)][s1] & s2) // NOTE: Currently working on this in asmLand (10/7)
			  {
				  PrintPieceType(pc);
				  pseudoAttacksHash = pseudoAttacksHash ^ (unsigned long long)(PseudoAttacks[type_of(pc)][s1]);
				  
				  std::cerr << " " << pseudoAttacksHash << " ";
				  // std::cerr << "#";
  ...
  std::cerr << std::endl << std::endl << "Final Hash for pseudoattacks is: " << pseudoAttacksHash << std::endl;
  assert(count == 3668);
}
